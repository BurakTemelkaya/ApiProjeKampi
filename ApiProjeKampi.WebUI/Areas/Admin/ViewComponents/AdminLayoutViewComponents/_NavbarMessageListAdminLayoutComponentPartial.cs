...
public class _NavbarMessageListAdminLayoutComponentPartial : ViewComponent
{
    ...
    public async Task<IViewComponentResult> InvokeAsync(CancellationToken cancellationToken = default)
    {
        ...
        HttpResponseMessage responseMessage = await httpClient.GetAsync("https://localhost:7051/api/Messages/MessageListByUnreadMessages", cancellationToken);
        if (responseMessage.IsSuccessStatusCode)
        {
            List<ResultMessageByUnreadDto>? data = await responseMessage.Content.ReadFromJsonAsync<List<ResultMessageByUnreadDto>>(cancellationToken);
            return View(data);
        }
        return View();
    }
